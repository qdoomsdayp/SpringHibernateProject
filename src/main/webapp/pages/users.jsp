...
<head>
    ...
    <meta cgarset="utf-8">
    <link rel="stylesheet" href="<c:url value='/pages/us.css'/>" type="text/css">
...
<form name="users" action="Users/form" method="post">
    ...
    <div class="main">
        <div class="UP">
            ...
            <a href="http://localhost:8080/roles">Роли</a>
        </div>
...
<script type="text/template" id="templateTable">
    <table id="myTable" class="table1">
        ...
        <tr class="table1 rows">
            <td class="table1"> {{=listU[i].nameU}}</td>
            <td class="table1"> {{=listU[i].emailU}}</td>
            {{for (var j = 0; j < listR.length; j++) { }}
            {{if (listU[i].idR == listR[j].idR) { }}
            <td class="table1" abbr=" {{=listR[j].idR}}   "> {{=listR[j].nameR}}</td>

            {{ } }}
            {{ } }}
            <td class="table1">
                <input type="button" name="{{=listU[i].idU}} " value="R" onclick="editRow(this,this.name,r)">
                <input type="button" name="{{=listU[i].idU}}" value="[X]" onclick="del(this.name,r)">
            </td>
            {{ } }}
        </tr>
    </table>
